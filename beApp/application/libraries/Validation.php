...
class Validation {
    ...
    function validate_client($client_id, $client_secret) {
        if ($client_id != "beapp") {
            $return = error_res("Invalid client",403);
            echo json_encode($return);
            die;
        }
        if ($client_secret != "QUWKTPFYU7J6DQFWEV9C31QSPZSSSS") {
            $return = error_res("Invalid client",403);
            echo json_encode($return);
            die;
        }
    ...
    function required_fields($required,$blank,$params){

  // $required is comma seperated required fileds
        if($required!=""){
           $keys=explode(",", $required);
        for($i=0;$i<count($keys);$i++){
          
            $this->is_parameter_blank($keys[$i],$params[$keys[$i]]);
          }  
        }
   // $blank is comma seprated required filed which set blank if not exits     
        if($blank!=""){
       
         $keys=explode(",", $blank);

         for($i=0;$i<count($keys);$i++){         
            $params[$keys[$i]]=$this->set_blank_parameter($params[$keys[$i]]);
          }
    ...
    function set_blank_fields($parameter,$params){
        $keys=explode(",", $parameter);
        for($i=0;$i<count($keys);$i++){
            $this->set_blank_parameter($keys[$i],$value[$keys[$i]]);
        }
    ...
    function is_parameter_blank($keyname, $parameter, $message = null) {
        ...
        if (strlen(trim($parameter)) == 0) {
            if ($message == null) {
                $return = error_res("Please enter " . $keyname,403);
            } else {
                $return = error_res($message,403);
            }
    ...
    function is_valid_email($keyname, $parameter, $message = null) {
        ...
        if (!filter_var($parameter, FILTER_VALIDATE_EMAIL)) {
            if ($message == null) {
                $return = error_res("Please enter valid " . $keyname,403);
            } else {
                $return = error_res($message,403);
            }
...
//////////// WEBSITE URL VALIDATION
    ...
    function file_upload($file_key, $folder_path,$custom_filename) {
